...
minetest.register_on_generated(function(minp, maxp, seed)
	...
	local perlin1 = minetest.get_perlin(27-minp.y, 3, 0.5, perlin_scale)	--Get map specific perlin
	local x0,z0,x1,z1 = minp.x,minp.z,maxp.x,maxp.z	-- Assume X and Z lengths are equal
	...
	local area = VoxelArea:new{MinEdge=emin, MaxEdge=emax}
 	local pos, pos2
	local depth = math.random(25, 35)
	local start = math.random(minp.y + 50, maxp.y - 3)

	for z=minp.z,maxp.z do
		for x=minp.x,maxp.x do
			local test = math.abs(perlin1:get2d({x=x, y=z}))
			if test <= 0.1 then
				for y=math.max(minp.y, math.floor(start+test*100-depth+0.5)),math.min(start+15, maxp.y) do
					local p_pos = area:index(x, y, z)
					pos = {x=x, y=y, z=z}
					data[p_pos] = c_air
				end
